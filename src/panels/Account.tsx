...
export const Account: React.FC = () => {
  ...
  const resume = async () => {
    try {
      const s = await authReconnect();
      if (s.signedIn) { setStatus(s); setPending(false); clearInterval(pollRef.current); toast('Resumed ' + (s.name || s.email) + '.'); }
      else toast('No saved session to resume — sign in with Google.', true);
    } catch { /* ignore */ }
  };
        ...
        <div className="acct-pop">
          {signedIn ? (
            ...
            <>
              ...
              <div className="acct-sub">Continue with Google — pick which account. Studio runs unlimited on your own Claude.</div>
              ...
              <button className="acct-link" onClick={resume}>Resume previous session</button>
            </>
          )}
        </div>
      )}
